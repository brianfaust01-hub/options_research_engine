...
- Remove contradictory presentation such as `CLOSE` beside `RAISE STOP`
- Make the controlling portfolio action the first and only immediate execution
  instruction for each position
- `CLOSE` now says to sell the full position and cancel remaining exit orders;
  target and stop guidance is explicitly marked no longer applicable
- `REDUCE` states exactly how many contracts to sell and retain, with target and
  stop guidance applying only to the remaining contracts
- `HOLD` states whether to keep or raise the stop and shows the applicable level
- Position actions are sorted `CLOSE`, `REDUCE`, then `HOLD` for faster execution
- Removed the verbose profit-protection and duplicate target/stop columns from
  the daily action table while preserving the underlying calculations and data

### Validation

- Fixture coverage proves a full close never displays stop-adjustment guidance
- Fixture coverage proves a reduction names both the sale and remaining quantity
- All 71 fixture tests pass
- No scoring, allocation, exit, or risk-management logic changed

---

## Sprint 41A.6 — Portfolio Timestamp Schema Reliability

### Incident

- The September 8 daily run stopped during position review when an entirely
  empty `PeakPremiumDate` CSV column was inferred as `float64`
- Pandas rejected the subsequent ISO timestamp assignment, so the daily report
  and email were not generated by the first run

### Implemented and Validated

- Portfolio schema loading now restores all schema-owned identifier, status,
  and timestamp fields to an assignment-safe object dtype
- Added a regression test covering empty timestamp and profit-protection fields
- All 70 fixture tests pass
- The end-to-end rerun completed at 11:08:50 on September 8, generated the
  Markdown and HTML reports, sent the email, and exited with code 0
- The weekday 10:30 scheduled task was re-enabled by the user; no production
  scoring or allocation rules changed

---

## Sprint 41A.5 — Current-Policy Readiness Baseline
...
- Changes were committed as `fd07b40` and pushed to `schwab-integration`
